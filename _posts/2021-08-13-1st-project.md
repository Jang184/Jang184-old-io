...
```
-------
# <span style='color:darkorange'>Products 앱 라우팅</span>
```
urlpatterns = [
...
## request.GET 으로 쿼리 파라미터 가져오기
```
order  = request.GET.get('order',1)
```
## 딕셔너리를 이용해 '번호:정렬종류' 로 연결하기
```
order_number = {
    1 : '-created_at',
    2 : '-sales',
...
## 상품번호를 이용해 특정 상품의 정보 가져오기
```
product = Product.objects.get(id=product_id)
```
-----
# <span style='color:darkorange'>Orders 앱 라우팅</span>
```
urlpatterns = [
...
```
## `get_or_create`를 사용해 Cart 객체 만들기
```
